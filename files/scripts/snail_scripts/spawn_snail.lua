...
local snail_count = #(EntityGetWithTag("ti_snail") or {})
local snail_count_max = tonumber(GlobalsGetValue("ti_snail_max_amount","0"))
snail_respawn_timer = snail_respawn_timer or 2
if snail == nil or snail == 0 or not EntityGetIsAlive(snail) or snail_count < snail_count_max then
	if snail_respawn_timer <= 0 then
		...
		if valid then
			snail_respawn_timer = 2
			EntityLoad("mods/twitch-integration/files/entities/animals/immortal_snail.xml", new_x, new_y)
