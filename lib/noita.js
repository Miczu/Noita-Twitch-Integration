...
class Noita {
    ...
    async doQuestion() {
        ...
        this.voting = true
        this.userVotes = {}
        if (this.settings.permanent_gamba_option) {
            this.outcomes.settings.gamba.enabled = false
            this.outcomes.settings.gamba.votes = 0
            this.choices = this.outcomes.drawByType(this.settings.options_per_vote - 1, this.settings.option_types)
            this.choices.push(JSON.parse(JSON.stringify(this.outcomes.settings.gamba)))
        } else {
